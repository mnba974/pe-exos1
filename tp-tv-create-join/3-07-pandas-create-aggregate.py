...
# %%
animals = np.array([['snail', 0.1, 2.0],
        ['pig', 17.5, 8.0],
        ['elephant', 40.0, 70.0],
        ['rabbit', 48.0, 1.5],
        ['giraffe', 52.0, 25.0],
...
# %%
df= pd.DataFrame(animals,columns=['names','speed','lifespan'])
df[df.names=='elephant'].lifespan

# %%
df[df.names=='elephant'].names

# %%
df.iloc[2].loc['names']='grey elephant'
df[df.names=='grey elephant'].names
...
# %%
df=df.astype({'speed':float, 'lifespan': float})
df.dtypes
...
# %% tags=["level_intermediate"]
r= df1i.merge(df2i, left_index=True,right_index=True)
pd.concat([r,df3i])
