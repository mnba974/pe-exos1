...
empty_columns = [x for x in df.columns if is_empty_column(df,x)]
...
df.drop(labels=[x for x in df.columns if df[x].isna().sum() == len(df[x])], axis=1, inplace=True)
```
...
df.drop(labels=[x for x in df.index if df.cLT2FREQ.isna().iloc[x]], axis=0 , inplace=True)
```
