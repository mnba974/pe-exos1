...
# %%
po=open('rgb-codes.txt','r')
a=po.read()
a=a.split()
a=np.array(a)
a=np.reshape(a,(len(a)//4,4))

d={}
for i in range(len(a)):
    d[a[i][0]]=list(a[i][1:].astype('int'))
...
def sepia(im):
    ...
    im=im/255
    return im
q=plt.imread('patchwork-all.jpg')
q=sepia(q)
plt.imshow(q)

w=plt.imread('les-mines.jpg')
w=sepia(w)
plt.imshow(w)
...
z=Image.open('les-mines.jpg')
y=plt.imread('les-mines.jpg')
plt.imsave('mine2.png',y) #1.12mb
z.save('mine3.png',quality=100) #993kb
